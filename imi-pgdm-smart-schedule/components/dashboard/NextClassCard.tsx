...
interface NextClassCardProps {
  ...
  /** Subject code → {name, faculty}, auto-fetched from the sheet's legend tab. */
  subjectNames: Record<string, SubjectInfo>;
}

export function NextClassCard({ state, subjectNames }: NextClassCardProps) {
  ...
  const primaryInfo = primaryEntry
    ? resolveSubjectInfo(subjectNames, primaryEntry.subjectCode)
    : { name: '', faculty: '' };
      ...
      <div className="mt-6 flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="font-display text-3xl leading-none font-extrabold tracking-wide uppercase sm:text-4xl">
            {primaryEntry ? subjectSelectionKey(primaryEntry) : 'Class'}
          </p>
          {primaryEntry?.room && (
            <p className="text-muted mt-2 text-sm">Room {primaryEntry.room}</p>
          )}
          {primaryInfo.faculty && (
            <p className="text-muted mt-1 text-sm">Faculty: {primaryInfo.faculty}</p>
          )}
          {extraCount > 0 && (
