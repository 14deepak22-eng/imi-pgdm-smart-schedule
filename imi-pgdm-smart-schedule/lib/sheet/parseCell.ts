...
for (const keyword of EVENT\_KEYWORDS) {
if (lower.includes(keyword)) {
return EVENT\_CATEGORY\_MAP[keyword] ?? 'other';
}
}
return null;
}
...
const BASE\_CODE\_PATTERN = /^[A-Z]{2,4}\d{3}/;
...
const codes = CANONICAL\_SUBJECT\_CODES\_BY\_BATCH[batchPrefix] ?? [];
const list = codes
...
- batch's authoritative subject list (CANONICAL\_SUBJECT\_CODES\_BY\_BATCH),
...
const baseMatch = part.match(BASE\_CODE\_PATTERN);
...
let match = rest.match(NEXT\_GROUP\_PATTERN);
while (match) {
...
match = rest.match(NEXT\_GROUP\_PATTERN);
}
...
.map((part) => part.trim())
.filter(Boolean)
.some((part) => BASE\_CODE\_PATTERN.test(part));
}
